...
class ScoreInquiry(commands.Cog):
	...
	def get_score(self, game_id):
		if game_id != None:
			...
			if response.ok:
				...
				home = game['header']['competitions'][0]['competitors'][0]
				away = game['header']['competitions'][0]['competitors'][1]
				return self.create_score_string(game, home, away)
			else:
				response.raise_for_status()
	...
	def create_score_string(self, game, home, away):
		if 'score' in home:
		  result = '-----------------\n'
		  result += away['team']['displayName'] + ': ' + away['score'] + '\n'
		  result += home['team']['displayName'] + ': ' + home['score'] + '\n'
		  result += '-----------------'
		else:
			result = '-----------------\n'
